...
- [Input Guardrails (INPUT_GUARD)](#input-guardrails-inputguard)
- [Output Guardrails (OUTPUT_GUARD)](#output-guardrails-outputguard)
- [Zero-Leak Buffered Mode (ZERO_LEAK_GUARD)](#zero-leak-buffered-mode-zeroleakguard)
- [Guardrail Authoring Factories (GUARD_FACTORY)](#guardrail-authoring-factories-guardfactory)
- [Language Guard (LANG_GUARD)](#language-guard-langguard)
- [Hate Speech Guard (HATE_SPEECH_GUARD)](#hate-speech-guard-hatespeechguard)
...
- [Pipeline Orchestrator (GUARD_PIPELINE)](#pipeline-orchestrator-guardpipeline)
...
flowchart TB
    ...
    subgraph InputLayer["Input Layer (INPUT_GUARD)"]
    ...
    subgraph OutputLayer["Output Layer (OUTPUT_GUARD)"]
    ...
    subgraph OutputAction["Output Action"]
        DEV_TRIP["Dev: abort throws TripWire"]
        PROD_SUPP["Prod: suppress + inject fallback"]
...
flowchart LR
    subgraph Dev["development mode"]
        D_P0["p0 output hit"]
        D_EMIT["Emit tripwire event\nwith conceptId + details\nStream terminates"]
        D_P0 --> D_EMIT
    end

    subgraph Prod["production mode"]
        P_P0["p0 output hit"]
        P_SUPPRESS["Suppress ALL remaining chunks"]
        P_INJECT["Inject text-delta chunk\nwith fallback from ConceptRegistry[conceptId]"]
        P_P0 --> P_SUPPRESS --> P_INJECT
    end

    NOTE["p1 behavior is identical\nin both modes:\npass + log to Langfuse"]
    NOTE_P0_INPUT["p0 input behavior is identical\nin both modes:\nabort() always"]
```
...
flowchart TB
    ...
    subgraph Parallel["Run ALL guardrails in parallel"]
...
flowchart TB
    ...
    subgraph GuardrailRun["Run guardrails on window text"]
...
flowchart TB
    ...
    subgraph BufferPhase["Buffer Phase (N tokens)"]
    ...
    subgraph StreamPhase["Stream Phase (post-flush)"]
...
```mermaid
flowchart TB
    subgraph Factories["Guardrail Factories (library)"]
...
```mermaid
flowchart TB
    subgraph Composite["createCompositeGuardrail([g1, g2, g3], 'worst-wins')"]
        INPUT_C["text input"]
        
        subgraph Inner["Inner guardrails (parallel)"]
            G1_C["Regex guardrail\n→ p2 PASS"]
            G2_C["Keyword guardrail\n→ p1 flagged"]
            G3_C["LLM guardrail\n→ p2 PASS"]
        end
    ...
    INPUT_C --> G1_C & G2_C & G3_C
    G1_C & G2_C & G3_C --> AGG_C --> RESULT_C
```
...
- Parallel execution rule: both engines run in parallel on every input and every output sliding window evaluation. Any match returns p0.
...
flowchart TB
    ...
    subgraph ParallelEngines["Run in parallel"]
...
```mermaid
flowchart TB
    subgraph Verdicts["Collected Verdicts"]
...
```mermaid
flowchart TB
    subgraph PipelineOrchestrator["GuardrailPipelineOrchestrator"]
        CONFIG["GuardrailPipelineConfig"]
        
        subgraph InputGuardrails["Framework InputGuardrail[]"]
            PI_HOOK["execute(context, agent)"]
            PI_RUN["Run input guardrails\nin parallel"]
            PI_AGG["Worst-wins aggregation"]
            PI_ABORT["tripwireTriggered: true on p0"]
            PI_FLAG["onFlag() on p1"]
        end
        
        subgraph OutputGuardrails["Framework OutputGuardrail[]"]
            PO_HOOK["execute(context, agent, output)"]
            PO_WINDOW["Update sliding window"]
            PO_RUN["Run output guardrails\non window text"]
            PO_AGG["Worst-wins aggregation"]
            PO_ACTION["Dev: tripwireTriggered throws\nProd: suppress + fallback"]
        end
    end
    ...
    InputGuardrails -->|wired as| AGENT_IN["agent.inputGuardrails[]"]
    OutputGuardrails -->|wired as| AGENT_OUT["agent.outputGuardrails[]"]
```
...
sequenceDiagram
    ...
    Client->>InputProc: User message

    Note over InputProc: Run all input guardrails in parallel

    alt p0 detected
        InputProc->>Client: TripWire (abort)
    else p1 detected
        InputProc->>Langfuse: onFlag(verdict)
        InputProc->>Agent: Pass message
    else p2
        InputProc->>Agent: Pass message
    end
    ...
    loop Each streaming chunk
        LLM->>OutputProc: text-delta chunk
        Note over OutputProc: Update window, run guardrails

        alt p0 + development
            OutputProc->>Client: Tripwire chunk with details
        else p0 + production
            OutputProc->>Client: Suppress + inject fallback
        else p1
            OutputProc->>Langfuse: onFlag(verdict)
            OutputProc->>Client: Chunk (pass through)
        else p2
            OutputProc->>Client: Chunk (pass through)
        end
    end
...
flowchart TB
    ...
    subgraph GuardrailInput["Input Guardrails (INPUT_GUARD)"]
        IG["Run before intent detection\nProtects against prompt injection"]
    end

    subgraph IntentDetection["Intent Detection (10)"]
        ID["Classify intents\nRewrite queries"]
    end

    POST_INTENT_GATE["Post-intent language gate\nPost-Intent Gate enforcement"]

    subgraph Orchestrator["Orchestrator Agent (05)"]
        ORCH_AGENT["Supervisor agent\nSpawns sub-agents"]
        
        subgraph SubAgents["Sub-Agents (not individually guardrailed)"]
    ...
    subgraph GuardrailOutput["Output Guardrails (OUTPUT_GUARD)"]
        OG["Run on orchestrator synthesis stream\nSliding window buffer"]
    end

    CLIENT["Client (SSE)"]

    USER_MSG --> GuardrailInput
    GuardrailInput -->|passes| IntentDetection
    IntentDetection --> POST_INTENT_GATE
    POST_INTENT_GATE --> Orchestrator
    SUB_AGENT_ONE & SUB_AGENT_TWO --> SYNTH
    SYNTH --> GuardrailOutput
    GuardrailOutput --> CLIENT
```
...
**The evidence gate (file 12) is a separate concern.** It evaluates whether retrieved evidence is sufficient to answer a question. It's not a safety guardrail and doesn't use the `GuardrailFn` interface.

### Relationship to Observability (file 16)
...
### Relationship to Streaming (file 13)
...
In development mode, the TripWire exception carries `conceptId`, `reason`, and `fallbackMessage` (matching the `SSETripwireEvent` shape from file 13). The TUI and development clients catch and render with a visual indicator (error banner + fallback message).
...
|----------|-------------|
| **Configuration** ([02](./02-configuration.md)) | Provides model, environment, and policy configuration values that parameterize guardrail behavior and runtime mode selection. |
| **Agent & Orchestration** ([05](./05-agent-and-orchestration.md)) | Wires guardrail processors into agent creation and orchestrator streaming so safety checks run before and during responses. |
| **Memory System** ([07](./07-memory-system.md)) | Memory deletion guardrail purges stale cache entries in Valkey when facts are deleted from SurrealDB via memoryDelete tool. |
| **Intent Detection** ([10](./10-intent-and-routing.md)) | Supplies the generateObject schema and decision output that LANG_GUARD Post-Intent Gate uses for post-intent intended output language enforcement. |
| **Server Implementation** ([14](./14-server-implementation.md)) | Supplies server-owned guardrail definitions and ConceptRegistry mappings that the library executes through this pipeline. |
| **Observability & Eval** ([16](./16-observability-and-eval.md)) | Consumes guardrail `onFlag` callbacks and traces p0/p1 safety events for monitoring and evaluation. |
| **Constraints & Success** ([20](./20-constraints-and-success.md)) | Defines the non-negotiable safety constraints and quality targets this guardrail architecture must enforce. |
...
*Previous: [05 — Agent & Orchestration](./05-agent-and-orchestration.md)*
