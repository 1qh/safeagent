...
The factory accepts an `errorMessageMap` parameter — a plain object keyed by error code string, where values are either a static message string or a function `(metadata) => string` for dynamic messages. When the handler catches an error and emits an `error` SSE event, it looks up the error's code in this map to populate the `message` field. If the code is not in the map, a generic fallback message is used. The server passes its error message map (defined in file 14) when constructing the handler. This is the DI mechanism that keeps the library language-agnostic while letting the server control user-facing tone.
...
Its primary use is injecting file context in direct mode, where the user has uploaded files that should be visible to the agent without going through the full RAG pipeline. The server implements the function to fetch file content from S3, read from a local cache, or return nothing based on whether the request includes file references.
...
|----------|-------------|
| **Agent & Orchestration** ([05](./05-agent-and-orchestration.md)) | Defines orchestrator and processor-chain behavior, including location tool orchestration, that produces the `RunStreamEvent` stream consumed by this SSE transport layer. |
| **Guardrails & Safety** ([06](./06-guardrails-and-safety.md)) | Defines language drift detection in output sliding windows and p0 enforcement behavior used by this transport layer. |
| **Server Implementation** ([14](./14-server-implementation.md)) | Owns the Elysia route wiring and HTTP boundary where this document's `createStreamHandler` and SSE event protocol are applied. |
| **TUI App** ([15](./15-tui-app.md)) | Consumes `RunStreamEvent` streaming directly, while this document defines the parallel SSE/client transport path for non-TUI clients. |
...
*Previous: [12 — File Intelligence](./12-file-intelligence.md)*
*Next: [14 — Server Implementation](./14-server-implementation.md)*
