...
> **Scope**: Langfuse integration via the `@openai/agents` framework's pluggable `TracingExporter` interface + direct `langfuse` SDK, custom observability spans for guardrails / RAG / file processing, user feedback scoring, Langfuse prompt management, custom evaluation scorer configuration, and Promptfoo self-test infrastructure.
>
...
When `LANGFUSE_PUBLIC_KEY` is not set, the entire observability module returns no-ops — `exporter` is `null`, `scoreHelper` methods silently do nothing. No consumer needs null checks.

---

## Trace Hierarchy

Every agent interaction produces a trace in Langfuse. Core instrumentation populates the top-level structure. Custom spans add domain-specific detail at each stage.
...
`createObservability` reads configuration from its argument or falls back to environment variables. It constructs a `Langfuse` client from `langfuse`, builds the `LangfuseTracingExporter`, attaches PII redaction via `@logtape/redaction`, and returns both components.
...
| `@logtape/redaction` | PII redaction utilities applied before telemetry export. |
...
| `numeric` | accepts traceId, optional spanId, name, value, optional comment | Custom scorer results, custom metrics |

The `spanId` parameter is optional (`string | undefined`). When `undefined`, the score attaches to the trace itself rather than a specific span. User feedback always passes `undefined` for `spanId` because feedback applies to the entire interaction.

### No-Op Behavior

When `LANGFUSE_PUBLIC_KEY` is absent, the factory returns fully functional no-ops for both return values. `exporter` is `null`. `scoreHelper.boolean` and `scoreHelper.numeric` are silent no-ops. This means every consumer — the agent runtime, feedback endpoint, custom spans — can destructure and call methods without any null checks or conditional logic.
...
All spans pass through custom redaction logic built with `@logtape/redaction` before export. This strips PII (email addresses, phone numbers, etc.) from span attributes. Redaction is enabled by default in `createObservability` and can be disabled for debugging via config.
...
| Score target | Entire trace (not a specific span — `spanId` is `undefined`) |
...
| `get` | Returns a compiled prompt string. Serves from cache when fresh. On cache miss or expiry, attempts refresh then falls back. Supports variable interpolation (`{{var}}`). |
| `refresh` | Force-refreshes a specific prompt or all prompts from Langfuse. |
| `stats` | Returns cache diagnostics: `{ cacheSize, lastRefreshAt? }`. |
...
1. **Langfuse outage at startup**: `preload` logs a warning and loads all fallback prompts. Agent starts normally.
...
6. **Timeout**: Remote fetch calls use `AbortController` to bound network latency. Never blocks the critical path indefinitely.
...
|----------------|----------|
| Relevance | `answerRelevancy`, `promptAlignment` |
...
|-----------|-------------|
| **Agent & Orchestration** ([05](./05-agent-and-orchestration.md)) | `createAgent` runtime consumes tracing helpers. Agents emit traces for every run. |
| **Guardrails** ([06](./06-guardrails-and-safety.md)) | Input and output guardrail processors accept optional `scoreHelper` for custom spans. `createGuardrailFlagCallback` provides the p1 flag callback without coupling guardrail code to Langfuse. |
| **RAG & Retrieval** ([09](./09-rag-and-retrieval.md)) | RAG pipeline spans trace embed_query → hybrid_search → fetch_context → answer. Search arm scores captured for relevance debugging. |
| **Document Processing** ([08](./08-document-processing.md)) | File processing spans trace split_pages → summarize → enrich. Per-page concurrency and token usage captured. |
| **Memory System** ([07](./07-memory-system.md)) | Memory operation spans trace fact storage, recall search, graph traversal, and TTL cleanup. |
| **Server** ([14](./14-server-implementation.md)) | Server destructures `{ exporter, scoreHelper }` from `createObservability`. Feedback endpoint uses `scoreHelper`. Prompt manager injected into agent config. |
| **Configuration** ([02](./02-configuration.md)) | `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL` control whether observability is active. |
| **Infrastructure** ([17](./17-infrastructure.md)) | Langfuse stack (6 services) defined in Docker Compose under `profiles: [langfuse]`. Shares Postgres and MinIO instances. |
...
**What to do**: Build the `LangfuseTracingExporter` that implements the `@openai/agents` framework's `TracingExporter` interface. The exporter translates framework `Trace` and `Span` objects into Langfuse API calls via the direct `langfuse` SDK. Register it with the framework using `addTraceProcessor(new BatchTraceProcessor(langfuseExporter))`. Build `createObservability` factory that sets up the exporter, score helper, and PII redaction. Return `{ exporter, scoreHelper }`. Implement `createScoreHelper` with `boolean` and `numeric` methods — `spanId` parameter is optional for trace-level scores. Default to env vars. Enable PII redaction (`@logtape/redaction`) by default. Implement the full no-op path when `LANGFUSE_PUBLIC_KEY` is absent: call `setTracingDisabled(true)` on the framework, `scoreHelper` with silent no-op `boolean` and `numeric`. Dev mode uses `realtime: true`, production uses `realtime: false` with configurable ratio sampling.
...
- Streaming guardrail emits `guardrail_output_pass` boolean score
...
**QA Scenarios**:
- Create input guardrail with mock `scoreHelper` → process safe input → assert boolean called with guardrail_input_pass and true → process toxic input → assert called with false
...
- Create guardrail without `scoreHelper` (undefined) → process input → assert no error, guardrail works normally, no span/score creation attempted
...
> **FEEDBACK_ENDPOINT** — canonical task specification is in [14 — Server Implementation](./14-server-implementation.md#task-feedback_endpoint-feedback-endpoint). The server route wires Langfuse score submission to the HTTP layer. See also LANGFUSE_MODULE for the scoreHelper dependency.
...
- `get` returns compiled prompt with variable interpolation (this refers to the Langfuse prompt revision identifier)
...
**QA Scenarios**:
- Mock Langfuse API returns prompt `system-prompt` → create manager with fallback → call `preload` → call `get` → assert returned prompt comes from remote template with interpolation applied
- Mock fetch fails with timeout → create manager with fallback → call `preload` and `get` → assert fallback prompt returned, warning logged without secrets
...
*Previous: [15 — TUI App](./15-tui-app.md)*
*Next: [17 — Infrastructure](./17-infrastructure.md)*
