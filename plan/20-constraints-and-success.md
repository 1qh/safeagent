# 20 — Constraints & Success Criteria
...
- [Language and content safety](#language-and-content-safety)
- [Must NOT Have — Complete Exclusions](#must-not-have-complete-exclusions)
- [Runtime Clarification](#runtime-clarification)
- [Conventions](#conventions)
- [Final Verification Pipeline (AUDIT_PLAN, AUDIT_CODE, AUDIT_QA, AUDIT_SCOPE)](#final-verification-pipeline-auditplan-auditcode-auditqa-auditscope)
...
| ID | Requirement | Detail |
|---|---|---|
| MH_CROSS_CONV_RAG | Cross-conversation RAG | `scope: 'global' | 'thread'` on uploads — global docs accessible across all threads per user, thread results ranked higher in hybrid search |
| MH_ADMIN_API | Admin API | Admin budget detail, update, and list endpoints — per-user budget management with Valkey cache invalidation, admin role required |
| MH_PROMPT_MGMT | Langfuse prompt management | `createPromptManager` factory — fetches prompts from Langfuse API at startup, caches with TTL, falls back to local prompts |
| MH_ZERO_LEAK | Zero-leak output guardrail | `mode: 'buffered'` on output guardrails — buffers N tokens (default 50), suppresses entire response if guardrail triggers during buffer, zero partial leaks |
| MH_OFFLINE_QUEUE | Client SDK offline queue | `offline: { enabled: true }` — queues messages when network unavailable, drains FIFO on reconnect, bounded queue with overflow events |
| MH_SURQLIZE | surqlize ORM for SurrealDB | All SurrealDB queries use surqlize — type-safe RELATE, traversal, vector search. No raw SurrealQL strings |
| MH_TYPED_ENV | Typed environment variables | `@t3-oss/env-core` with Zod v4 schemas — compile-time typed env access, startup validation with descriptive errors |
| MH_TYPED_ERRORS | Typed error results at boundaries | `neverthrow` Result types for DB operations, external API calls, guardrail execution, file validation. Callers forced to handle both paths |
| MH_NO_RAW_SQL | Raw SQL prevention | `eslint-plugin-drizzle` + Biome — type-safe Drizzle query builder enforced. `sql` template tag permitted only in designated query modules |
| MH_PRECOMMIT_HOOKS | Pre-commit quality gates | `husky` + `lint-staged` — Biome check runs on staged files before every commit. Broken code never enters the repository |
| MH_WATCH_ONLY | Watch-only development | Watch mode for dev server and tests. Hot mode is banned due to native module bugs and debugger resource leaks |
| MH_OPENAPI_DOCS | OpenAPI documentation | `@elysiajs/openapi` — auto-generated OpenAPI spec from Zod v4 route schemas. JSON and UI served at dedicated OpenAPI endpoints. Zero duplication between validation and documentation |
| MH_SEED_DATA | Development seed data | Idempotent seed scripts in both repos. Library seeds SurrealDB memory graph, server seeds PostgreSQL + MinIO. Scoped to dedicated test user |
| MH_TYPEDOC | Library API documentation | `typedoc` generates searchable API docs from TypeScript source. All public exports documented |
| MH_BUN_LINK | Local development linking | Local workspace linking for consuming safeagent from server during development. Published package used in CI |

### Complete Must-Have → Task Ownership Mapping

Every must-have requirement maps to one or more implementation tasks in the [execution plan](./19-execution-plan.md). This table is the definitive traceability matrix — no must-have is unowned.
...
flowchart LR
    subgraph MUST_HAVE["MUST HAVE — Inside Boundary"]
        ...
        AREA_GUARD[TUI<br/>1 item]
        AREA_RAG[Evaluation<br/>2 items]
        AREA_DOC_PROC[Document Processing<br/>10 items]
        AREA_TOOL_EXEC[Storage & Files<br/>7 items]
        AREA_COST[Observability<br/>5 items]
        AREA_TELEMETRY[Data Layer<br/>1 item]
        AREA_CONFIG[CTA Streaming<br/>4 items]
        AREA_SSE[Location Enrichment<br/>4 items]
        AREA_WEBSOCKET[Infrastructure<br/>6 items]
        AREA_RATE_LIMIT[Cross-Cutting<br/>15 items]
        AREA_HEALTH[Language and Content Safety<br/>5 items]
...
| **All AUDIT_PLAN, AUDIT_CODE, AUDIT_QA, AUDIT_SCOPE APPROVE** | Final gate | All four verdicts are APPROVE | Project complete |

---

*Previous: [19 — Execution Plan](./19-execution-plan.md)*
