...
- [Full Dependency Matrix](#full-dependency-matrix)
- [Subpath Barrel Export Convention](#subpath-barrel-export-convention)
- [Batch Completion Rules](#batch-completion-rules)
- [New Task Registry (Documents 05, 06, 10, 11, 12)](#new-task-registry-documents-05-06-10-11-12)
...
gantt
    ...
    Batch 0 — SPIKE_CORE_STACK Core stack validation spike          :crit, w0, 2024-01-01, 2d
    Batch 0.5 — SPIKE_RAG_DEPS RAG Spike               :crit, w05, after w0, 2d

    section Scaffolding
    Batch 1 — Repo Setup (2 parallel)           :w1, after w05, 1d

    section Foundation
    Batch 2 — Foundation A (8 parallel)          :w2, after w1, 3d
    Batch 3 — Foundation B (13 parallel)         :w3, after w2, 3d

    section Config + Guards
    Batch 4 — Config+Guards (6 parallel)         :w4, after w3, 2d

    section Agent Factory + Pipelines
    Batch 5 — Agent Factory + Pipelines (4 parallel)      :crit, w5, after w4, 3d

    section Integration
    Batch 6 — Integration Layer (17 parallel)    :w6, after w5, 3d

    section Self-Test + Mid Integration
    Batch 7 — Self-test + Mid Integration (8 parallel):w7, after w6, 2d

    section Server + TUI + Pipeline
    Batch 8a — Server+TUI+Pipeline (16 parallel) :w8a, after w7, 3d
    Batch 8b — Extended Integration (12 parallel) :w8b, after w8a, 2d

    section Routes + API
    Batch 9a — Server Routes + Sub-Agent (2 parallel)      :crit, w9a, after w8b, 2d
    Batch 9b — Endpoints+Barrel (5 parallel)     :w9b, after w9a, 1d

    section Testing + Deploy
    Batch 10 — E2E+Deploy+Publish (4 parallel)   :w10, after w9b, 3d

    section Final Audit
    FINAL — Compliance+QA (4 parallel)          :crit, wf, after w10, 2d
```
...
> **NEW**: REWRITE_STRATEGIES (from Document 11) delivers three independently importable rewrite strategy modules. Depends only on types — fits naturally alongside other CORE_TYPES-dependent tasks.
...
> **NEW**: FILE_REGISTRY (from Document 12) implements per-user file reference resolution (Postgres + Valkey cache). EVIDENCE_GATE (from Document 12) implements the structural anti-hallucination gate with Attribute-First citation planning and is scheduled in Batch 7.
...
> **NEW**: EMBED_ROUTER (Document 10) implements the fast semantic classifier using Valkey-cached embeddings. RAGFLOW_CLIENT (Document 11) wraps RAGFlow's retrieval API. LOCATION_TOOL (Document 05) adds geocoding and image enrichment with streamed location events. DOC_SEARCH and VISUAL_GROUNDING are scheduled in Batch 8a because both require EVIDENCE_GATE (Batch 7).
...
> **NEW**: LLM_INTENT (Document 10) implements the LLM authority that always validates the embedding router's guess and performs conditional query rewriting. SOURCE_ROUTER (Document 11) implements parallel source execution with priority-weighted result merging.
...
### New Task Dependency Chain (Documents 10, 11, 12)

```mermaid
graph TB
    subgraph IntentPipeline["Intent Detection Pipeline (Document 10)"]
    ...
    subgraph QueryPipeline["Query Pipeline (Document 11)"]
    ...
    subgraph FileIntelligence["File Intelligence (Document 12)"]
    ...
    subgraph Orchestration["Orchestration (Document 05)"]
...
> (new) = New task from expanded requirements (Documents 05, 06, 07, 10, 11, 12, 14)
...
```mermaid
flowchart TB
    subgraph Rule1["Rule 1: Sequential Batches"]
        W_N["Batch N"]
        W_N1["Batch N+1"]
    ...
    subgraph Rule2["Rule 2: Parallel Within Batch"]
        START["Batch starts"]
        T_A["Task A"]
        T_B["Task B"]
        T_C["Task C"]
    ...
    subgraph Rule3["Rule 3: Failure Isolation"]
        FAIL_T["Task X fails"]
...
## New Task Registry (Documents 05, 06, 07, 10, 11, 12)
...
| DEPENDENT_INTENT | Dependent intent coordination in orchestrator | Documents 05, 10 | 8b | `deep` | ORCHESTRATOR, LLM_INTENT |
| RESPONSE_CALIBRATION | Response length/energy matching signal computation | Document 05 | 8a | `unspecified-high` | CORE_TYPES, AGENT_FACTORY |
| CLARIFICATION_MODEL | Proactive clarification + multi-turn patience model | Document 05 | 8b | `deep` | LLM_INTENT, EMBED_ROUTER, AGENT_FACTORY |
| LANG_GUARD | Language Guard — two-stage language enforcement + output drift scanner | Document 06 | 6 | `deep` | CORE_TYPES, GUARD_FACTORY |
| HATE_SPEECH_GUARD | Hate Speech Guard — hybrid obscenity + multilingual matching | Document 06 | 6 | `deep` | CORE_TYPES, GUARD_FACTORY |
| LOCATION_TOOL | Location Enrichment Tool — geocoding, optional image enrichment, and streamed location events | Document 05 | 6 | `deep` | CORE_TYPES, AGENT_FACTORY, VALKEY_CACHE |
| EMBED_ROUTER | Embedding Router — vector similarity classifier | Document 10 | 6 | `deep` | CORE_TYPES, VALKEY_CACHE, AGENT_FACTORY |
| LLM_INTENT | LLM Intent Validator + Query Rewriter | Document 10 | 8a | `deep` | CORE_TYPES, AGENT_FACTORY, EMBED_ROUTER |
| FRUSTRATION_SIGNAL | Frustration escalation detection across turns | Document 10 | 8b | `deep` | EMBED_ROUTER, LLM_INTENT |
| PREFETCH_COORD | Speculative Pre-Fetch Coordinator | Document 10 | 8b | `deep` | EMBED_ROUTER, LLM_INTENT, SOURCE_ROUTER |
| RAGFLOW_CLIENT | RAGFlow Client + Tool — raw fetch wrapper | Document 11 | 6 | `deep` | CORE_TYPES, CONFIG_DEFAULTS |
| SOURCE_ROUTER | Source Priority Router — parallel fan-out + weighted merge | Document 11 | 8a | `deep` | RAGFLOW_CLIENT, CORE_TYPES, CONFIG_DEFAULTS |
| REWRITE_TOOL | Query Rewrite Tool — 7-trigger conditional rewriting | Document 11 | 8b | `unspecified-high` | REWRITE_STRATEGIES, CORE_TYPES, LLM_INTENT |
| REWRITE_STRATEGIES | Rewrite Strategy Modules — HyDE, EntityExtraction, DenseKeywords | Document 11 | 3 | `quick` | CORE_TYPES |
| FILE_REGISTRY | FileRegistry — temporal, ordinal, named resolution | Document 12 | 4 | `deep` | STORAGE_WRAPPER, VALKEY_CACHE |
| EVIDENCE_GATE | Evidence Bundle Gate — sufficiency scoring + thresholds | Document 12 | 7 | `deep` | EMBED_ROUTER, RAG_INFRA, CORE_TYPES |
| DOC_SEARCH | Per-Document Search Tool — searchDocument (evidence bundle) | Document 12 | 8a | `deep` | FILE_REGISTRY, EVIDENCE_GATE, RAG_INFRA |
| VISUAL_GROUNDING | Visual Grounding — multimodal LLM for charts/tables/images | Document 12 | 8a | `deep` | FILE_STORAGE, CONFIG_DEFAULTS, FILE_REGISTRY, EVIDENCE_GATE |
| ORCHESTRATOR | Orchestrator Agent Framework — supervisor + parallel sub-agent synthesis | Document 05 | 8b | `deep` | AGENT_FACTORY, EMBED_ROUTER, LLM_INTENT, SOURCE_ROUTER |
| SUBAGENT_FACTORY | Sub-Agent Factory — intent-scoped Agent + Handoff creation with tool assignment | Document 05 | 9a | `deep` | AGENT_FACTORY, ORCHESTRATOR, SOURCE_ROUTER |
...
| NON_ACTIONABLE_DETECT | Non-actionable message detection (pleasantries, gibberish short-circuit) | Document 10 | 7 | `quick` | EMBED_ROUTER |
| ATTRIBUTE_NEGATION | Attribute negation detection and search filtering | Documents 10, 11 | 8b | `unspecified-high` | LLM_INTENT, REWRITE_TOOL |
| QUERY_REPLAY | Query replay detection and rewriting | Documents 10, 11 | 8b | `unspecified-high` | LLM_INTENT, REWRITE_TOOL, STRUCTURED_RESULT_MEM |
...
| CONTEXT_BUDGET | Context window budget management with truncation | Documents 05, 07 | 8a | `unspecified-high` | SHORT_TERM_MEM, USER_SHORTTERM_MEM, FACT_EXTRACTION, MEMORY_RECALL |
| SUMMARY_CAP | Rolling summary size cap with compaction | Document 07 | 4 | `quick` | SHORT_TERM_MEM |
| INPUT_VALIDATION | Input message length validation | Document 14 | 7 | `quick` | SSE_STREAMING |

---

*Covers all 99 implementation tasks + 4 final audit tasks = 103 total across 15 execution batches. Includes 33 tasks from expanded requirements (Documents 05, 06, 07, 10, 11, 12) and 8 engine-gap tasks, all fully integrated into the batch structure and dependency matrix.*

---

*Previous: [18 — Testing Strategy](./18-testing-strategy.md)*
*Next: [20 — Constraints & Success Criteria](./20-constraints-and-success.md)*
