...
> **Scope**: Agent creation factory wrapping `@openai/agents`, SDK bridge helper for Gemini, orchestrator with handoff-based sub-agent routing, multi-intent handling, live synthesis streaming, provider fallback, computer use and browser automation patterns, MCP client integration, location enrichment tooling, generative UI component governance, and context shaping behaviors that make responses feel natural under real conversational conditions.
>
