...
- **JWT secret absent in production** (`NODE_ENV=production`): hard failure. Authentication is a security boundary, and auth fail-open would allow unauthorized data access. The server refuses to start. This is the only non-Postgres hard failure (see [12 — Server](./12-server.md)).
...
This strategy aligns with [03 — Architecture](./03-architecture.md), [04 — Foundation](./04-foundation.md), [12 — Server](./12-server.md), and [14 — Observability](./14-observability.md).
---
## API Key Pool
